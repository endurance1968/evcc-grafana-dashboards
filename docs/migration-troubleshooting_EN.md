...
Use this document only when the normal path in [influx-to-vm-migration.md](./influx-to-vm-migration_EN.md) reports a problem.
...
If no `evcc_*` series exist, rerun the rollup backfill and scheduler setup from [influx-to-vm-migration.md](./influx-to-vm-migration_EN.md).