...
Important: generation-cost values are only meaningful when mapped PV energy is complete per `evcc_title`. If a year contains only a few days with per-title energy while investment costs run for the full year, ct/kWh values are artificially high. Check such periods through the import summary or VictoriaMetrics before interpreting the values.

## Separate PV Cost Workflow

SMA imports and generation-cost calculation are separate steps by design. An SMA importer writes only SMA data; the investment helper calculates costs only from metrics that already exist in VictoriaMetrics. This keeps each step testable and repeatable on its own.

Typical flow:

1. optionally import system-wide SMA energy balance for old years,
2. optionally import per-source SMA daily PV yields,
3. calculate PV generation costs from EVCC, SMA, or a combination of both energy sources.

### 1. Optional SMA Energy-Balance Import
...
This step writes historic EVCC-compatible daily rollups for total PV, home, grid, and battery. It does not generate PV cost metrics.

### 2. Optional SMA PV Daily-Yield Import
...
This step writes the EVCC-compatible daily metric `evcc_pv_energy_by_title_daily_wh`. The mapping file maps SMA names to `evcc_title` so the values can later match the investment rows.

### 3. Calculate Generation Costs

From existing EVCC PV data only:
...
From already imported EVCC-compatible per-title daily yields only; the standard metric is `evcc_pv_energy_by_title_daily_wh`:
...
EVCC and SMA combined, with EVCC winning on overlapping days and SMA filling gaps only:
...
`import-pv-generation-costs.py` remains only as a cost wrapper for existing automations. It no longer imports SMA files; SMA data must already exist in VictoriaMetrics through the SMA import scripts.
