...
For the SMA PV importer, `--replace` deletes only the target metric selected through `--metric`. The default is `evcc_pv_energy_by_title_daily_wh`. This is intended for controlled refresh imports and does not change EVCC raw data such as `pvPower_value` or investment/generation-cost metrics generated by the investment helper.
...
The SMA PV importer stays a pure importer. It writes daily yields per `title` to `evcc_pv_energy_by_title_daily_wh` and does not calculate further metrics. The investment helper may read this energy as input, but it does not write it itself.
