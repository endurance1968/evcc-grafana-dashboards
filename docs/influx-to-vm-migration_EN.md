...
Before running commands, review the central requirements overview: [system-requirements.md](./system-requirements_EN.md).

This is the normal end-user path from an existing EVCC + InfluxDB setup to VictoriaMetrics.

This guide intentionally covers only the green path. If a check fails, continue with [migration-troubleshooting.md](./migration-troubleshooting_EN.md). For release and energy-calibration background, see [migration-validation-notes.md](./migration-validation-notes_EN.md).
...
If the check reports missing or drifting data, stop and use [migration-troubleshooting.md](./migration-troubleshooting_EN.md) before building rollups.
...
If the recommendation is `REVIEW` or `STOP`, use [migration-troubleshooting.md](./migration-troubleshooting_EN.md). Do not build rollups on top of a partially cleaned raw import.
...
Use [migration-checklist.md](./migration-checklist_EN.md) before removing InfluxDB from the active dashboard path.
