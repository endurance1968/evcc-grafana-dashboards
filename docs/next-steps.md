...
- the optimized read-side consolidation is now the active production path for `evcc_*`
...
- after aligning tariff carry-over with the old 15-minute lookback semantics, the remaining delta versus `evcc_*` is now in the promille range: `grid_import_daily_wh` is still off by about `888.93 Wh` in 2025 = about `0.12‰`, `vehicle_charge_cost_daily_eur` by about `0.0015 EUR`, and `grid_import_cost_daily_eur` by only about `0.0383 EUR` in 2025 = about `0.022‰`
- because RAM stayed acceptable and the remaining delta is now already in the promille range, the separate slower `compat` fetch mode has been removed; the remaining tuning knob is `max_fetch_points_per_series` for smaller chunk windows on weaker hosts
